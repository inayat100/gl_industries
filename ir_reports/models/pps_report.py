...
class PPSLab(models.Model):
    ...
    article_url = fields.Char(string="ARTICLE URL", tracking=True)
    new_article_no = fields.Char(string="New ARTICLE NO", tracking=True)
    po_url = fields.Char(string="PO URL", tracking=True)
    ...
    season_id = fields.Many2one("season.master", string="Season", tracking=True)
    ...
    pps_lab_lines = fields.One2many("pps.lab.line", "pps_lab_id", string="Lines", tracking=True)
    route_id = fields.Many2one("pps.route", string="Route", tracking=True)
